...
Take a polyglot from mitra, make the file encrypt to another valid file via AES-CBC with a crafted IV
"""
...
if __name__=='__main__':
	parser = argparse.ArgumentParser(
		description="Turn a non-overlapping polyglot into a file staying valid after AES-CBC encryption.")
	...
	plain1 = dIn[:BLOCKLEN]
	# print("first plaintext block", plain1)

	cipher1 = other_hdr + dIn[len(other_hdr):BLOCKLEN]
	# print("other plaintext block:", cipher1)

	ecb_dec = AES.new(key, AES.MODE_ECB)
	cipher1 = ecb_dec.decrypt(cipher1)
	# print("Other after decryption:", cipher1)

	iv = bytearray([cipher1[i] ^ plain1[i] for i in range(BLOCKLEN)])
	iv_s = b2a(binascii.hexlify(iv))

	cbc_enc = AES.new(key, AES.MODE_CBC, iv)
	dOut = cbc_enc.encrypt(dIn[:swap]) + dIn[swap:]

	cbc_dec = AES.new(key, AES.MODE_CBC, iv)
	dOut = cbc_dec.decrypt(pad(dOut))
